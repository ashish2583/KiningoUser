...
import { baseUrl, shop_eat_cart, shop_eat_cart_id, shop_eat_coupons_userid, shop_eat_cart_apply_coupon, login, shop_eat_business, shop_eat_business_id, shop_eat_menu_userid, requestPostApi, requestGetApi, shop_eat, shop_product_business } from '../../../WebApi/Service'
...
const ShopProductFilter = (props) => {
  ...
  //     {id: '6', title: 'Paleo'}, 
  //   ])
  const [filterData, setFilterData] = useState(props.route.params.categories?.map((el, index) => { return { ...el, id: String(index + 1), title: el.category_name } }))
  ...
  const getData = async (urls) => {
    ...
    var fUrl = shop_product_business
    if (urls != undefined) {
      fUrl = fUrl + urls
    }
    const { responseJson, err } = await requestGetApi(fUrl, '', 'GET', User.token)
    setLoading(false)
    console.log('the res get shop_product_business ==>>', responseJson.body)
  ...
  const makeUrl = (val) => {
    if (val == 'Rating') {
      ...
      var murl = '?lat=' + '28.5355' + '&long=' + '77.3910' + '&sortBy=rating_desc'
      return murl
    } else if (val != 'Rating') {
      var murl = '?lat=' + '28.5355' + '&long=' + '77.3910' + '&category_type=' + val
      return murl
    } else {
      return undefined
    }
    ...
    <SafeAreaView style={{ flex: 1, }}>
      ...
      >
        ...
        <View style={{ width: '92%', alignSelf: 'center' }}>
          <View>
          <ProductSearchInput marginTop={10} placeholder={'Search By Vendor Name'}
            serchValue={searchValue}
            onChangeText={(e) => {
              setsearchValue(e)
              //   var murl='?name='+e.text+'&lat='+mapdata.restorentlocation.latitude+'&long='+mapdata.restorentlocation.longitude
              var murl = '?name=' + e.text + '&lat=' + '28.5355' + '&long=' + '77.3910'
              getData(murl)
            }}
            press={() => { Alert.alert('Hi') }}
            presssearch={() => {
              //   var murl='?name='+searchValue.text+'&lat='+mapdata.restorentlocation.latitude+'&long='+mapdata.restorentlocation.longitude
              var murl = '?name=' + searchValue.text + '&lat=' + '28.5355' + '&long=' + '77.3910'
              getData(murl)
            }}
            isCustomSearchIcon
            customSearchIcon={<AntDesign name="filter" color={'#FFF'} size={24} style={{ alignSelf: 'center' }} />}
            paddingLeft={9} />
                  ...
                  renderItem={({ item, index }) => {
                    return (
                      <View style={{ marginHorizontal: 5, }}>
                        <TouchableOpacity style={[{ justifyContent: 'center', padding: 10, borderRadius: 5 }, selected.includes(item) ? styles.categorySelectedStyle : null]}
                          onPress={() => {
                          ...
                          <Image source={{ uri: item.category_image }} style={{ width: "100%", height: 40, alignSelf: 'center', borderRadius: 7 }}></Image>
                          <Text style={{ fontSize: 11, color: selected.includes(item) ? '#835E23' : Mycolors.Black, marginTop: 5, textAlign: 'center', fontWeight: 'bold' }}>{item.title}</Text>
                        </TouchableOpacity>
                      </View>
                    )
                  }}
                      ...
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 5 }}>
                        ...
                        <View style={{ padding: 5, alignItems: 'flex-end' }}>
                          <TouchableOpacity style={{
                            width: 50, height: 28, borderRadius: 5, backgroundColor: '#835E23',
                            shadowColor: '#000',
                            shadowOffset: {
                              width: 0,
                              height: 3
                          ...
                          <Text style={{ fontSize: 11, color: Mycolors.ORANGE, marginTop: 5, textAlign: 'left', fontWeight: '500', }}>{item.total_orders == '0' ? 'No' : item.total_orders + '+'} products sold.</Text>

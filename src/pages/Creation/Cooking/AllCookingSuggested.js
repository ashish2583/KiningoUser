...
//         console.log('my url home page searcg---------->', art_HomePage)
...
//             // Toast.show({ text1: responseJson.headers.message });
//         } else {
//             Toast.show({ text1: 'No Data found' });
//             setCategorydata([]);
...
import { useSelector, useDispatch } from 'react-redux';
import Loader from '../../../WebApi/Loader';
import { baseUrl, shop_eat_cart, user_payment_method, shop_eat_orders, shop_eat_cart_book_dining, shop_eat_cart_book_table, shop_eat_cart_id, shop_eat_business_id, shop_eat_menu_userid, requestPostApi, requestGetApi, shop_eat, connect_people_create_post, connect_people_newPost, art_getCollection, requestPostApiMedia, art_PostCollection, postApiWithToken2, art_HomePage, art_likeDislike, get_suggestion, creation_searchHome, creation_home, creation_getView } from '../../../WebApi/Service'
...
const AllCookingSuggested = (props, route) => {
    const dispatch = useDispatch();
    const User = useSelector(state => state.user.user_details)
    console.log('User', User.token);
                                                        ...
                                                        <View>
                                                            ...
                                                            <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10 }} onPress={() => {
                                                                ...
                                                                <View style={{
                                                                    ...
                                                                    <Text style={styles.descriptionTextrrrr}>{item.headline}</Text>
                                                                    <Text style={styles.createdTimeText}>{item.category
                                                                    }</Text>
                                                                </View>
                                                            </TouchableOpacity>
                                                            <View style={styles.flatlistBottomView}>
...
export default AllCookingSuggested 